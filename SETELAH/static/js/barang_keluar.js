...
document.addEventListener('DOMContentLoaded', function() {
        ...
        fetch('/api/inventory')
            ...
            .then(items => {
                ...
                if (inventoryDatalist) {
                    ...
                    items.forEach(item => {
                        if (item.id) { // Pastikan item memiliki ID
    ...
    function updateStockInfo() {
        ...
        if (item && item.id) { // Pastikan item valid dan memiliki ID
            ...
            stockInfo.textContent = selectedItemId ? 'Item ID tidak ditemukan.' : 'Pilih item ID yang valid untuk melihat stok.';
            stockInfo.className = 'form-text text-warning';
            
            // Disable quantity input if no valid item selected
            if (quantityInput) {
